...
UIFont *plv_chat_text_font() {
    return [UIFont systemFontOfSize:14.0];
}

#pragma mark - Private Classes

@interface PLVChatLabel : UILabel

@property (assign, nonatomic) UIEdgeInsets edgeInsets;

@end

@implementation PLVChatLabel
...
- (void)setupChatUser:(PLVChatUser *)chatUser {
    [self.avatarImgView sd_setImageWithURL:[NSURL URLWithString:chatUser.avatar] placeholderImage:[UIImage imageNamed:@"plv_img_default_avatar"]];
    if (chatUser.actor) {
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    self = [super initWithStyle:style reuseIdentifier:reuseIdentifier];
    if (self) {
        self.contentLabel = [[PLVChatLabel alloc] init];
        self.contentLabel.numberOfLines = 0;
        self.contentLabel.font = plv_chat_text_font();
        self.contentLabel.edgeInsets = UIEdgeInsetsMake(10, 10, 10, 10);
...
- (void)layoutCell {
    ...
    self.actorLable.hidden = self.isMyMessage;
    self.nickNameLabel.hidden = self.isMyMessage;
    self.avatarImgView.hidden = self.isMyMessage;
    
    PLVChatModel *chatModel = (PLVChatModel *)self.model;
    self.contentLabel.attributedText = chatModel.textContent.attributedText;
    
    if (self.myMessage) {
        self.contentLabel.textColor = [UIColor colorWithWhite:51.0 / 255.0 alpha:1.0];
        self.contentLabel.backgroundColor = UIColorFromRGB(0xafcbec);
        
        CGSize newSize = [self.contentLabel sizeThatFits:CGSizeMake(270, MAXFLOAT)];
        chatModel.cellHeight = newSize.height + 10; // 10
        
        [self drawCornerRadiusWithView:self.contentLabel size:newSize roundingCorners:UIRectCornerTopLeft|UIRectCornerBottomLeft|UIRectCornerBottomRight];
        ...
        self.contentLabel.textColor = chatModel.textContent.textColor;
        self.contentLabel.backgroundColor = [UIColor whiteColor];
        
        CGSize newSize = [self.contentLabel sizeThatFits:CGSizeMake(260, MAXFLOAT)];
        chatModel.cellHeight =  newSize.height + 33; // 10+18+5
        
        [self drawCornerRadiusWithView:self.contentLabel size:newSize roundingCorners:UIRectCornerBottomLeft|UIRectCornerBottomRight|UIRectCornerTopRight];
        [self.contentLabel mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.size.mas_equalTo(newSize);
    ...
    switch (chatModel.type) {
        case PLVChatModelTypeMySpeak: {
            ...
            return [self autoCalculateSize:CGSizeMake(270, MAXFLOAT)
                         attributedContent:chatModel.textContent.attributedText].height + 30.0;
        } break;
        case PLVChatModelTypeOtherSpeak: {
            ...
            return [self autoCalculateSize:CGSizeMake(260, MAXFLOAT)
                         attributedContent:chatModel.textContent.attributedText].height + 53.0;
        } break;
        default: break;
    }
    return 0.0;
...
- (void)layoutCell {
    ...
    self.nickNameLabel.hidden = self.isMyMessage;
    self.avatarImgView.hidden = self.isMyMessage;
    self.loadingBgView.hidden = !self.isMyMessage;
