...
- (void)addControllView {
    ...
    [self.arrowBtn setImage:[PCCUtils getLinkMicImage:@"plv_arrow"] forState:UIControlStateNormal];
    [self.arrowBtn setImage:[PCCUtils getLinkMicImage:@"plv_arrowed"] forState:UIControlStateSelected];
...
- (UIButton *)addButtonOnControlViewWithNormalImgName:(NSString *)normalImgName selectedImgName:(NSString *)selectedImgName action:(SEL)action {
    ...
    if (normalImgName && normalImgName.length > 0) {
        [btn setImage:[PCCUtils getLinkMicImage:normalImgName] forState:UIControlStateNormal];
    }
    if (selectedImgName && selectedImgName.length > 0) {
        [btn setImage:[PCCUtils getLinkMicImage:selectedImgName] forState:UIControlStateSelected];
    }
    if (action) {
...
- (void)refreshLinkMicBtnStatus {
    ...
    switch (self.linkMicStatus) {
        case PLVLinkMicStatusDisabe: {
            ...
            [self.linkMicBtn setImage:[PCCUtils getLinkMicImage:@"plv_linkMic1_disable"] forState:UIControlStateNormal];
            break;
        }
        case PLVLinkMicStatusNone: {
            self.tipLabel.textColor = [UIColor whiteColor];
            self.tipLabel.text = @"讲师已开始连线";
            [self.linkMicBtn setImage:[PCCUtils getLinkMicImage:self.fullscreen ? @"plv_linkMic2" : @"plv_linkMic1"] forState:UIControlStateNormal];
            break;
        }
        case PLVLinkMicStatusWait: {
            self.tipLabel.textColor = [UIColor whiteColor];
            self.tipLabel.text = @"请等待讲师允许";
            [self.linkMicBtn setImage:[PCCUtils getLinkMicImage:self.fullscreen ? @"plv_linkMic2ed" : @"plv_linkMic1ed"] forState:UIControlStateNormal];
            break;
        }
        ...
        case PLVLinkMicStatusJoin: {
            ...
            [self.linkMicBtn setImage:[PCCUtils getLinkMicImage:@"plv_linkMic2ed"] forState:UIControlStateNormal];
            break;
        }
        default:
            break;
