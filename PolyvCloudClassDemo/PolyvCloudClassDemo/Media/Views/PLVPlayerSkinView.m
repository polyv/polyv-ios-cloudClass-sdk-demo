...
- (void)timelabelSizeToFit {
    if (self.currentPlayTimeLabel.text.length > 0) {
        ...
        UIEdgeInsets currentPlayTimeMargin = UIEdgeInsetsMake(-1.0, 54.0, 0.0, -1.0);
        UIEdgeInsets blankMargin = UIEdgeInsetsMake(-1.0, 54.0 + w, 0.0, -1.0);
        UIEdgeInsets durationMargin = UIEdgeInsetsMake(-1.0, 64.0 + w, 0.0, -1.0);
...
- (void)layout {
    ...
    if (self.type == PLVPlayerSkinViewTypeLive) {
        UIEdgeInsets linkMicMargin = UIEdgeInsetsMake(-1.0, -1.0, 128.0, 10.0);
...
- (void)pan:(UIPanGestureRecognizer *)gestureRecognizer {
    ...
    if (gestureRecognizer.state == UIGestureRecognizerStateBegan) {
        ...
        if (fabs(velocty.x) > fabs(velocty.y)) {//左右滑动seek播放
            if (self.type == PLVPlayerSkinViewTypeVod && self.duration > 0.0 && !self.sliderDragging) {
        ...
        switch (self.panType) {
            case PLVPlayerSkinViewPanTypeSeekPlay: {
                ...
                if (gestureRecognizer.state == UIGestureRecognizerStateEnded || gestureRecognizer.state == UIGestureRecognizerStateCancelled) {
                    [self sliderTouchEnd:self.slider];
                    [self hiddenPopView];
                }
                break;
            }
...
- (IBAction)switchDanmu:(id)sender {
...
- (void)showMessage:(NSString *)message {
    UILabel *messageLabel = [self addLabel:message fontSize:17.0 textAlignment:NSTextAlignmentCenter inView:self];
...
- (IBAction)mainTap:(id)sender {
    if (self.type == PLVPlayerSkinViewTypeLive) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(refreshLive:)]) {
            [self.delegate refreshLive:self];
        }
    } else {
...
- (IBAction)changeSpeeed:(UIButton *)sender {
...
- (IBAction)changeCodeRate:(UIButton *)sender {
    [self selectItem:sender];
    ...
    for (NSUInteger i = 0; i < count; i++) {
        ...
        [btn mas_remakeConstraints:^(MASConstraintMaker *make) {
            if (weakSelf.fullscreen) {
                ...
                make.centerX.mas_equalTo(weakSelf.popView.mas_centerX);
            } else {
...
- (IBAction)speed:(id)sender {
    ...
    for (NSString *speedValue in speedItems) {
        UIButton *btn = [self addButton:nil selectedImgName:nil title:speedValue fontSize:18.0 action:@selector(changeSpeeed:) inView:self.popView];
...
- (IBAction)codeRate:(id)sender {
    [self addPopView:NO];
    
    for (NSString *codeRateValue in self.codeRateItems) {
        UIButton *btn = [self addButton:nil selectedImgName:nil title:codeRateValue fontSize:18.0 action:@selector(changeCodeRate:) inView:self.popView];
        [btn setTitleColor:BlueColor forState:UIControlStateSelected];
